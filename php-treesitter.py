...
class PHPASTAnalyzer:
    ...
    def _check_ssrf(self, func: Node, tracker: TaintTracker):
        """Detect SSRF via curl, file_get_contents, etc. with tainted URLs."""
        body = self._get_body(func)
        if not body:
            return

        # Check direct function sinks: file_get_contents($url), curl_init($url)
        func_calls = find_nodes(body, "function_call_expression")
        for call in func_calls:
            name_node = get_child_by_type(call, "name")
            if not name_node:
            ...
            if func_name in self.SSRF_SINKS:
                args = get_child_by_type(call, "arguments")
                if not args:
                    continue
                first_arg = self._get_first_arg(args)
                ...
                # Skip file_get_contents("php://input") — that's a taint source, not SSRF
                if func_name == "file_get_contents" and "php://input" in arg_text:
                    continue

                if tracker.is_tainted(arg_text) or \
                   self._has_tainted_concat(first_arg, tracker):
                    self._add_finding(
                        line, 0,
                        f"SSRF - {func_name}() with tainted URL",
                        VulnCategory.SSRF, Severity.HIGH, "HIGH",
                        tracker.get_taint_chain(arg_text),
                        f"User-controlled URL in {func_name}() can reach internal services."
                    )

            # curl_setopt($ch, CURLOPT_URL, $tainted)
            if func_name == "curl_setopt":
                all_args = self._get_all_args(get_child_by_type(call, "arguments") or call)
                if len(all_args) >= 3:
                    opt_text = node_text(all_args[1])
                    if "CURLOPT_URL" in opt_text:
                        val_text = node_text(all_args[2])
                        if tracker.is_tainted(val_text):
                            self._add_finding(
                                line, 0,
                                "SSRF - curl_setopt() with tainted CURLOPT_URL",
                                VulnCategory.SSRF, Severity.HIGH, "HIGH",
                                tracker.get_taint_chain(val_text),
                                "User-controlled URL in curl CURLOPT_URL can reach internal services."
                            )
    ...
    def _check_path_traversal(self, func: Node, tracker: TaintTracker):
        """Detect path traversal via filesystem functions with tainted unsanitized paths."""
        body = self._get_body(func)
        if not body:
            return
        ...
        for call in func_calls:
            ...
            if not name_node:
                continue
            func_name = node_text(name_node)
            if func_name not in self.PATH_TRAVERSAL_SINKS:
                continue
            ...
            # For move_uploaded_file, check the destination (2nd arg)
            if func_name == "move_uploaded_file":
                all_args = self._get_all_args(args)
                if len(all_args) < 2:
                    continue
                arg_text = node_text(all_args[1])
                first_arg = all_args[1]

            if not tracker.is_tainted(arg_text, category="PATH"):
                continue

            # Skip if path sanitization (basename/realpath) is present
            ...
            self._add_finding(
                ...
                tracker.get_taint_chain(arg_text),
                f"User-controlled path in {func_name}() without basename()/realpath() sanitization."
            )
